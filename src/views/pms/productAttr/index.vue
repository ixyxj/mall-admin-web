    ...
    <el-dialog
      ...
      <el-form ref="productAttrCatForm" :model="productAttrCate" :rules="rules" label-width="120px">
        <el-form-item label="类型名称" prop="typeName">
          <el-input v-model="productAttrCate.typeName" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    export default {
        ...
        methods: {
            ...
            handleUpdate(index, row) {
                ...
                this.productAttrCate.typeName = row.typeName;
                this.productAttrCate.id = row.id;
            },
            getAttrList(index, row) {
                this.$router.push({path: '/pms/productAttrList', query: {cid: row.id, cname: row.typeName, type: 0}})
            },
            getParamList(index, row) {
                this.$router.push({path: '/pms/productParamList', query: {cid: row.id, cname: row.typeName, type: 1}})
            },
            afterDialogRequest(formName) {
                        ...
                        if (this.dialogTitle === "添加类型") {
                            createProductAttrCate(this.productAttrCate).then(response => {
                                if (checkSuccess(response)) {
                                    ...
                                    this.afterDialogRequest(formName);
                                }
                            });
                        } else {
